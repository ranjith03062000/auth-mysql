...
class SignInandUpController extends Controller
{
    ...
    public function register(Request $request){
       ...
       /*this is password Encryption code */
        ...
        $recordSubmit        = DB::insert($insertQuery);
        if($recordSubmit){
    ...
    public function forgetPassword(Request $request){
        ...
        /*get user id */
        $userRecord = DB::table('users')->where('email_id',$data['email'])->select('user_id')->get();
        ...
        $update             =  User::where('user_id',$userRecord[0]->user_id)->update($updatePassword); 
